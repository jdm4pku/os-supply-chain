...
def plot_box(os_version):
    ...
    data_sections = ["0-cluster"]
    plt.figure(figsize=(10, 6))  # 创建一个图形对象，用于绘制所有箱线图
    position = 1  # 初始箱线图位置
    legend_labels = []  # 存储图例标签
    colors = ['lightblue','lightcoral', 'lightsalmon', 'lightpink']  # 每个文件的颜色
    ...
    for data_sec in data_sections:
        ...
        for i, (os_name, os_arch, os_ver) in enumerate(os_version):
            ...
            fig_path = os.path.join(dir_path, "result_box_test.png")
            ...
            # 绘制每个数据文件的箱线图
            plt.boxplot(scores, positions=[position], vert=False, patch_artist=True, meanline=True, boxprops=box_props)
            plt.xlabel('Value')
            plt.grid(True)
